...
class DeepQNetwork(nn.Module):
    def __init__(self, n_states, input_dims, n_actions, hidden1_dims, hidden2_dims, filename,
                 chkpt_dir, device=DEVICE):
        super(DeepQNetwork, self).__init__()

        # Initialize the network
        self.fc_nets = [
            nn.Sequential(
                nn.Linear(n_states, hidden1_dims//input_dims),
                nn.ReLU(),
                nn.Linear(hidden1_dims//input_dims, hidden2_dims//input_dims),
                nn.ReLU(),
                nn.Linear(hidden2_dims//input_dims, 1)
            )
            for _ in range(input_dims)
        ]
        self.fc_all = nn.Linear(input_dims, n_actions)
    ...
    def forward(self, state):
        ...
        if state.ndim == 2:
            for s in range(state.shape[1]):
                state_ohe = np.zeros([state.shape[0], self.n_states], dtype=int)
                for b in range(state.shape[0]):
                    state_ohe[int(state[b, s].item())] = 1
                proper_input = torch.tensor(state_ohe, dtype=torch.float32).to(self.device)
                vals.append(self.fc_nets[s](proper_input))
            vals = torch.cat(vals, dim=1)
        else:
            for s in range(state.shape[0]):
                state_ohe = np.zeros(self.n_states, dtype=int)
                state_ohe[int(state[s])] = 1
                state = torch.tensor(state_ohe, dtype=torch.float32).to(self.device)
                vals.append(self.fc_nets[s](state))
            vals = torch.cat(vals, dim=0)
        return self.fc_all(vals)
...
class Agent:
    def __init__(self, env, input_dims, n_actions, n_states, gamma, epsilon, eps_min, eps_dec,
                 n_episodes, lr=1e-3, batch_size=64, hidden1_dims=256, hidden2_dims=256, mem_size=100000,
                 chkpt_dir='./tmp/sdqn'):
        self.env = env
        ...
        self.q_net = DeepQNetwork(n_states, input_dims, n_actions, hidden1_dims, hidden2_dims,
